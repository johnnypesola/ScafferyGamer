...
if (!isDedicated) then {
	...
	player_death = compile preprocessFileLineNumbers "custom\death\player_death.sqf";	// Custom preserve skin script
	...
	player_selectSlot = compile preprocessFileLineNumbers "custom\extra_rc\ui_selectSlot.sqf";	// custom
...
DZ_KeyDown_EH = compile preprocessFileLineNumbers "custom\compile\keyboard.sqf";
...
// Fix anti TP for admin
[] spawn {
	waitUntil {!isNil "sched_antiTP"};

	sched_antiTP_old = sched_antiTP;
	sched_antiTP = {
		private["_lastpos","_lastheight","_lasttime","_lastVehicle","_debug"];
		...
		[_this select 0, _lastpos, _lastheight, _lasttime, _lastVehicle, _debug] call sched_antiTP_old;
	};
};
