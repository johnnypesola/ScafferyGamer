 ...
 * By Pastorn 2019
 */
private ["_proceed","_itemIn","_countIn","_missing","_missingQty","_qty","_removed","_tobe_removed_total","_textMissing","_num_removed","_removed_total","_objectID","_objectUID","_location","_dir","_objectCharacterID","_classname","_gunType","_upgradeType","_vehicle","_findNearestVehicles","_findNearestVehicle","_IsNearVehicle","_finished","_temp_removed_array_mag","_temp_removed_array_wep","_requirementsList"];

if (dayz_actionInProgress) exitWith {localize "STR_EPOCH_PLAYER_52" call dayz_rollingMessages;};
dayz_actionInProgress = true;

_requirementsList = [
	["M240_DZ","m107_DZ","KSVK_DZE","M32_EP1"],
	["MAAWS","RPG7V"],
	["Stinger","Strela"]
];

_gunType = _this select 0;
_upgradeType = _this select 1;
...
_findNearestVehicles = player nearEntities [["Air"],10];
_findNearestVehicle = [];
{
...
if (_IsNearVehicle >= 1) then {
	...
	if (!isNull _vehicle && local _vehicle && !_notNearestPlayer) then {
		...
		if (!(_classname in ["AN2_DZ"])) exitWith {dayz_actionInProgress = false; localize "STR_EPOCH_ACTIONS_18" call dayz_rollingMessages;};

		_requirementsWeapon = [[(_requirementsList select _gunType) select (_upgradeType-1), 1]];
		diag_log format ["We need %1 as payment!", _requirementsWeapon select 0];
		//_requirementsMagazine = _upgrade select 2;
		_requirementsMagazine = [];
		...
		if (_proceed) then {
			...
			if (_tobe_removed_total == _removed_total) then {
				...
				if(_objectID == "0" && _objectUID == "0") then {
					...
					dze_waiting = nil;
					PVDZE_veh_Upgrade = [_vehicle,[_dir,_location],_classname,true,_objectCharacterID,player,dayz_authKey,[_gunType, _upgradeType]];
					...
					format[localize "str_crafting_failed",_classname] call dayz_rollingMessages;
					} else {
