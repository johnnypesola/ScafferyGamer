...
if (isNil "keyboard_keys") then {
    ...
    // custom stuff
    _scaff_viewdist = {
        scaffery_viewDist = ((scaffery_viewDist + scaffery_viewDistIncrement) mod scaffery_viewDistLimit);
        setViewDistance (scaffery_viewDist+500);
        format["Changed view distance to %1", scaffery_viewDist+500] call dayz_rollingMessages;
        _handled = true;
    };

    _scaff_toggleDebug = {
        scaffery_toggle_showDebugMonitor = !scaffery_toggle_showDebugMonitor;
        if (scaffery_toggle_showDebugMonitor) then {
            "Debug monitor is on" call dayz_rollingMessages;
        } else {
            "Debug monitor is off" call dayz_rollingMessages;
        };
        _handled = true;
    };
