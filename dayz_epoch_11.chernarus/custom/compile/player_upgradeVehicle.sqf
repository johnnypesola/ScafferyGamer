...
if (count (crew _vehicle) == 0) then {
	...
	if (local _vehicle && !_notNearestPlayer) then {
		...
		// Added checks for local CfgVehicles files
		_upgradeTmp = getArray (configFile >> "CfgVehicles" >> _classname >> "Upgrades" >> _upgrade);
