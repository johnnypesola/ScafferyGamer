...
call compile preprocessFileLineNumbers "travel\travel_config.sqf";
...
if (isServer) then {
	...
	player_sendAwayCrew = compile preprocessFileLineNumbers "travel\player_sendAwayCrew.sqf";
	player_boardCrew = compile preprocessFileLineNumbers "travel\player_boardCrew.sqf";
	...
	if (!travelMonitorRunning) then {
		[] spawn {
			...
			if (6 == count PVCDZ_plr_Login2) then {	// Adapted to ESS v3
				...
				diag_log format["Travel: Not enough variables received from PVCDZ_plr_Login2: %1", PVCDZ_plr_Login2];
			};};
				...
				if (_menu == -1) then {
					if (player_inTravelZone && player != vehicle player) then {
						if (player == driver vehicle player) then {
							_veh = vehicle player;
							_menu = _veh addaction [("<t color=""#cccc00"">" + ("Travel to...") +"</t>"),"travel\player_showTravelMenu.sqf","true",0,false,true,"",""];
						};
					};
				};
