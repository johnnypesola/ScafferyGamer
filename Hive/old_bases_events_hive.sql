-- ---------------------------------------------
-- Procedure to find old bases on Chernarus
-- ---------------------------------------------
DELIMITER ;;
...
BEGIN
        ...
        SELECT DISTINCT -1 AS base_id,
                ...
                ON JSON_EXTRACT(objs.Worldspace,'$[1][0]')=base.ws_x
                AND JSON_EXTRACT(objs.Worldspace,'$[1][1]')=base.ws_y
                WHERE LastUpdated < DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 90 DAY) AND
                Datestamp < DATE_SUB(CURRENT_TIMESTAMP, INTERVAL 119 DAY) AND
                Classname LIKE '%plastic%' AND base_id IS NULL;
        ...
        DO
                ...
                SELECT ObjectUID,
                        ...
                        IF(Classname='Plastic_Pole_EP1_DZ', IFNULL(CONCAT('"',JSON_EXTRACT(Worldspace, '$[2]'),'"'), '0'), '0') AS ws_ownerpuid,
                        IFNULL(JSON_EXTRACT(Worldspace, '$[3]'), '[]') AS ws_vect,
                        IF(JSON_TYPE(JSON_EXTRACT(Inventory, '$[0][0]'))='STRING', Inventory, IFNULL(JSON_EXTRACT(Inventory, '$[1]'), '[]')) AS inventory_magazines,
...
-- ---------------------------------------------
-- Procedure to find old bases on Napf
-- ---------------------------------------------
DELIMITER ;;
