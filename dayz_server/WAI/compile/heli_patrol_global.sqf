...
private ["_wpList","_gunner2","_gunner","_skillarray","_startingpos","_heli_class","_startPos","_helicopter","_unitGroup","_pilot","_skill","_position","_wp","_respawnTime"];
_position = _this select 0;
_startingpos = _this select 1;
_wpList = _this select 2;
_heli_class = _this select 3;
_skill = _this select 4;
_respawnTime = _this select 5;
...
{
	_wp = _unitGroup addWaypoint [_x, 1];
	_wp setWaypointType "MOVE";
	_wp setWaypointCompletionRadius 200;
} forEach _wpList;

_wp = _unitGroup addWaypoint [_wpList select 0, 1];
_wp setWaypointType "CYCLE";
_wp setWaypointCompletionRadius 200;

_helicopter addEventHandler ["killed", {[getPosATL (_this select 0)] call fn_generateCrashLoot;}];

// Return [<group>, <alive>, <respawn time in seconds>]
[_unitGroup, true, _respawnTime]
