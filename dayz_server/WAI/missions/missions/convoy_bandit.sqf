...
[_position,"Disabled Convoy"] execVM "\z\addons\dayz_server\WAI\missions\compile\banditmarkers.sqf";
[nil,nil,rTitleText,"A Soldier Convoy has been disabled! Check your map for the location!", "PLAIN",10] call RE;
...
while {_missiontimeout} do {
	...
	{if((isPlayer _x) AND (_x distance _position <= 150)) then {_playerPresent = true; /*[_x, "banditmissionclean"] call attack_intruder;*/};}forEach playableUnits;
...
if (_playerPresent) then {
	...
	diag_log format["WAI: Mission Soldier Convoy Ended At %1",_position];
	[nil,nil,rTitleText,"Survivors have secured the soldier convoy!", "PLAIN",10] call RE;
} else {
	...
	diag_log format["WAI: Mission Soldier Convoy timed out At %1",_position];
	[nil,nil,rTitleText,"Survivors did not secure the soldier convoy in time!", "PLAIN",10] call RE;
};
