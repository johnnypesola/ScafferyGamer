...
diag_log format["WAI: Mission Soldier Crash_Spawner Started At %1",_position];
...
[_position,"Chopper Crash"] execVM "\z\addons\dayz_server\WAI\missions\compile\banditmarkers.sqf";
[nil,nil,rTitleText,"Soldiers have crashed a chopper! Check your map for the location!", "PLAIN",10] call RE;
...
while {_missiontimeout} do {
	...
	{if((isPlayer _x) AND (_x distance _position <= 100)) then {_playerPresent = true; /*[_x, "banditmissionclean"] call attack_intruder;*/};}forEach playableUnits;
...
if (_playerPresent) then {
	...
	diag_log format["WAI: Mission Soldier Crash_Spawner Ended At %1",_position];
	[nil,nil,rTitleText,"Survivors have secured the soldier crash site!", "PLAIN",10] call RE;
} else {
	clean_running_bandit_mission = True;
	deleteVehicle _crash;
	{_cleanunits = _x getVariable "banditmissionclean";
	...
	diag_log format["WAI: Mission Soldier Crash_Spawner Timed Out At %1",_position];
	[nil,nil,rTitleText,"Survivors did not secure the soldier crash site in time!", "PLAIN",10] call RE;
};
