...
if (count _this < 4) exitWith {diag_log "Server_PublishObj error: Wrong parameter format";};
...
if ([_object, "Server"] call check_publishobject) then {
	...
	#ifdef OBJECT_DEBUG
	diag_log format["PUBLISH: Server created %1 with UID:%2 CID:%3 @%4 inventory:%5",_type,_objectUID,_characterID,((_worldspace select 1) call fa_coor2str),_inventory];
	#endif
} else {
	#ifdef OBJECT_DEBUG
