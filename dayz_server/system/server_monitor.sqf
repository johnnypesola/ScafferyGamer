private ["_nul","_result","_pos","_dir","_isOK","_countr","_objWpnTypes","_objWpnQty","_dam","_selection","_totalvehicles","_object","_idKey","_type","_ownerID","_worldspace","_inventory","_hitPoints","_fuel","_damage","_key","_query","_vehLimit","_hiveResponse","_objectCount","_codeCount","_data","_status","_val","_traderid","_retrader","_traderData","_id","_lockable","_debugMarkerPosition","_vehicle_0","_bQty","_vQty","_BuildingQueue","_objectQueue","_superkey","_shutdown","_res","_hiveLoaded","_continueLoading","_page","_pageSize","_vehicles","_numberOfVehicles","_i"];
...
if (isNil "dayz_entry") then {
	...
	if (isServer && isNil "sm_done") then {
		...
		_totalvehicles = 0;
		{
			...
			_hitPoints =	_x select 10;
			_fuel =		_x select 11;
			_damage = 	_x select 12;

			_inventory =	[_x select 8, _x select 7, _x select 9];
			...
			//diag_log format ["OBJ DB Inventory received: %1", _inventory];
			...
			if (_damage < 1) then {
				...
				switch (_type) do {
					...
					case "MAP_F_postel_manz_kov": {
						...
						_object setVariable ["playerSpawn", ((_inventory select 0) select 0) select 0, true];
					};
				...
				if (count _inventory > 0 && "MAP_F_postel_manz_kov" != typeOf _object) then {
