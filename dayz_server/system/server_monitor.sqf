...
"_query","_page","_pageSize","_continueLoading","_vehicles","_numberOfVehicles","_hiveResponse","_qty"]; // extDB2
...
{
	diag_log format["You are running an old version of extDB: %1 - Update extDB to version 35 or later",_result];
};
...
{
	diag_log format ["Could not add database: %1", _result];
};
...
if ([(_result select 0), 0] call BIS_fnc_areEqual) exitWith {
	diag_log format ["Failed to initialize database protocol: %1", _result];
};
...
_result = "getDateTime" call server_hiveReadWrite;
//_outcome = _result select 0;	// extDB2
//if (_outcome == "PASS") then {
if ("ARRAY" == typeName _result) then {
	...
	diag_log ["TIME SYNC: Local Time set to:", _date, "Fullmoon:",dayz_ForcefullmoonNights,"Date given by extDB2:",_result select 1]; // extDB2
	setDate _date;
	dayzSetDate = _date;
...
// extDB2
diag_log "HIVE: Clean old objects from database...";
_key = [dayZ_instance];
_query = ["cleanOldObjects", _key] call dayz_prepareDataForDB;
_query call server_hiveWrite;

diag_log "HIVE: Loading vehicles from database...";
...
while {_continueLoading} do 
{
	...
	if (_numberOfVehicles > 0) then 
	{
		for "_i" from 0 to _numberOfVehicles - 1 do 
		{
			...
			_myArray set [_qty,_hiveResponse];
			_qty = _qty + 1;
		};
	};
	_page = _page + 1;
	if (_numberOfVehicles < _pageSize) then
	{
...
//diag_log ("HIVE: Streamed " + str(_val) + " objects");	// extDB2
diag_log ("HIVE: Streamed " + str(_qty) + " objects");
...

{
	...
	//_worldspace = 	_x select 4;	// extDB2
...
diag_log format["HIVE: BENCHMARK - Server_monitor.sqf finished streaming %1 objects in %2 seconds (unscheduled)",_qty,diag_tickTime - _timeStart]; // extDB2
		...
		if (!isNil "_traderData") then {
			{
				...
				_key = ["getTraderItemsCount", [_traderid]] call dayz_prepareDataForDB;
