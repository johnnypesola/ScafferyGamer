...
private ["_unit","_crate1","_crate2","_crate3","_crate4","_crate5","_coords","_rnd"];
...
OKLoot = ["weapons","medical","supply","secret","cloth","gardening"];
...
if (_rnd > 16) then 
    {
        _rnd = _rnd - 16;
        _crate5 = createVehicle ["USLaunchersBox",[(_coords select 0) - 15, (_coords select 1) - 15,0],[], 0, "CAN_COLLIDE"]; 
        _loot = OKLoot select floor random count OKLoot; 
        [_crate5,_loot] ExecVM OKBoxSetup; 
        [_crate5] call OKProtectObj; 
        diag_log format ["[OK]: Debug: box 1 spawned at %1",_coords];
    };
if (_rnd > 8) then 
    {
        _rnd = _rnd - 8;    
        _crate4 = createVehicle ["SpecialWeaponsBox",[(_coords select 0) + 15, (_coords select 1) + 15,0],[], 0, "CAN_COLLIDE"];
        _loot = OKLoot select floor random count OKLoot;
...
if (_rnd > 1) then 
    {
        ...
        diag_log format ["[OK]: Debug: box 5 spawned at %1",_coords];
    };
