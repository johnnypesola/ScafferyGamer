...
"_allowed","_hasAccess","_uid","_myCharID","_isLocked","_kickCrew"];	// Scaffery: added kick crew option for vehicles
...
*/
	...
	if (_player_lockUnlock_crtl) then {
		if (s_player_lockUnlock_crtl < 0) then {
			...
			if (_isLocked) then {
				...
				if (_hasKey || _oldOwner) then {
					...
					s_player_lockUnlock_crtl = 1;
					if (s_player_kickCrewOption < 0) then {
						s_player_kickCrewOption = player addAction [format["Kick out crew from %1", _text], "custom\compile\fn_kickCrewOption.sqf",_cursorTarget, -10, false, true];
					};
				};
			};
		};	 
		...
		s_player_lockUnlock_crtl = -1;
		if (s_player_kickCrewOption > -1) then { player removeAction s_player_kickCrewOption; s_player_kickCrewOption = -1; };
	};
	...
	*/
	...
	s_player_manageDoor = -1;
	// SCAFFERY:
	player removeAction s_player_kickCrewOption;
