...
*/
private ["_isOk","_i","_objName","_started","_finished","_animState","_isMedic","_proceed","_itemOut","_countOut","_tree","_trees","_findNearestTree","_treeTypes","_index","_invResult","_treesOutput","_text"];
...
_treeTypes = ["MAP_pumpkin","MAP_p_Helianthus","fiberplant"];
...
_findNearestTree = [];
{
//	diag_log format["DEBUG FOUND TREES: %1 at distance %2", _x, (player distance _x)];
	if("" == typeOf _x) then {
		...
		if ((typeOf _x) in _treeTypes) then {
			if (alive _x) exitWith {
				_objName = typeOf _x;
				_findNearestTree set [(count _findNearestTree), _x];
				_index = _treeTypes find _objName;
				_itemOut = _treesOutput select _index;
				_countOut = 1 + floor (random 3);	// Gives between 1 and 3 bags of seeds back
			};
		};
	};
