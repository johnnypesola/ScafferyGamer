...
// Check that plot pole is present
...
if(_IsNearPlot == 0) then {
	...
	if(dayz_characterID == _ownerID) then {  //Keep ownership
		// owner can build bed within his plot
		_canBuildOnPlot = true;
...
{if ((_x getVariable ["playerSpawn","0"]) == _playerUID) exitWith {_bedObject=_x;};} foreach _existingBeds;
if (!isNull _bedObject) exitWith {DZE_ActionInProgress = false; cutText [format ["You already have a spawn bed at %1.", getPosATL _bedObject], "PLAIN DOWN"];};
...
if (_hasRequiredTools && _hasbuilditem) then {
	...
	cutText [format["Warning: You can only build one bed and must be destroyed in order to build another!"], "PLAIN DOWN"];
	...
	if(!canbuild) then { _cancel = true; _reason = "Cannot construct spawn beds in a trader city."; };
	...
	if(!_cancel) then {
		...
		if (_proceed) then {
			...
			if((_tobe_removed_total == _removed_total) && (_removed_total >= 1)) then {
				...
				cutText [format["You have built a spawn bed, you can respawn here if you die :)"], "PLAIN DOWN"];
