...
_isStaticM2 = typeOf cursorTarget in ["M2StaticMG"];
_isStaticDShKM = typeOf cursorTarget in ["DSHKM_RU"];
...
if(_isPZombie) then {
	...
	if((speed player <= 1) && hasBikeItem && _canDo) then {
		if (s_player_deploybike < 0) then {
			s_player_deploybike = player addaction[("<t color=""#b4b700"">" + ("Deploy Bike") +"</t>"),"custom\deconstructible\deploy.sqf","",5,false,true,"", ""];
		};
	} else {
	...
	if((_isBike) && _canDo) then {
		if (s_player_deploybike2 < 0) then {
			s_player_deploybike2 = player addaction[("<t color=""#b4b700"">" + ("Re-Pack Bike") +"</t>"),"custom\deconstructible\pack.sqf","",5,false,true,"", ""];
		};
	} else {
	...
	// Pack Static M2 Machinegun
	if ("ItemToolbox" in _weapons) then {
		_cursorTarget = cursorTarget;
		_ownerID = _cursorTarget getVariable ["CharacterID","0"];
		if(isNil "_ownerID") then {
			canPackMachinegun = false;
		} else {
			if(_ownerID == "0") then {
				canPackMachinegun = false;
			} else {
				canPackMachinegun = true;
			};
		};
	} else {
		canPackMachinegun = false;
	};
	if (_isStaticM2 && _canDo && (speed player <= 1) && canPackMachinegun) then {
		if (s_player_packMachinegun < 0) then {
			s_player_packMachinegun = player addaction["Pack M2 Machine Gun", "custom\staticmg\pack_machinegun.sqf", _cursorTarget, 5, false, true, "", ""];
		};
	} else {
		player removeAction s_player_packMachinegun;
		s_player_packMachinegun = -1;
	};
	if (_isStaticDShKM && _canDo && (speed player <= 1) && canPackMachinegun) then {
		if (s_player_packMachinegun < 0) then {
			s_player_packMachinegun = player addaction["Pack DShKM Machine Gun", "custom\staticmg\pack_machinegun.sqf", _cursorTarget, 5, false, true, "", ""];
		};
	} else {
		player removeAction s_player_packMachinegun;
		s_player_packMachinegun = -1;
	...
	_cursorTarget = cursorTarget;
	_isAH6J = typeOf _cursorTarget in ["AH6J_EP1"];
	_isH13Gunship = _cursorTarget isKindOf "pook_h13_gunship";
	_isH13Transport = typeOf _cursorTarget in ["pook_H13_transport_GUE"];
	_hasReammoTarget = false;

	if (_isAH6J && _canDo && (speed player <= 1) && (player distance _cursorTarget < 8)) then {
		_hasReammoTarget = true;
		if (s_player_addAmmoOption < 0) then {
			s_player_addAmmoOption = player addaction["Add M134 ammo", "custom\ammo_tools\add_ammo.sqf", [_cursorTarget, "4000Rnd_762x51_M134", 1, "2000Rnd_762x51_M134", 2, -1], 6, false, true, "", ""]; 
		};
	};
	if (_isH13Gunship && _canDo && (speed player <= 1) && (player distance _cursorTarget < 8)) then {
		_hasReammoTarget = true;
		if (s_player_addAmmoOption < 0) then {
			s_player_addAmmoOption = player addaction["Add M60 ammo (from M134 ammo)", "custom\ammo_tools\add_ammo.sqf", [_cursorTarget, "2000Rnd_762x51_M134", 1, "2000Rnd_762x51_M134", 1, -1], 6, false, true, "", ""]; 
		};
	};
	if (!_hasReammoTarget) then {
		player removeAction s_player_addAmmoOption;
		//player removeAction s_player_addAmmoOption2;
...
if (!isNull cursorTarget && !_inVehicle && !_isPZombie && (player distance cursorTarget < _allowedDistance) && _canDo) then {	//Has some kind of target
	...
	_isModularItem = _cursorTarget isKindOf "ModularItems" or _cursorTarget isKindOf "Land_DZE_WoodDoor_Base" or _cursorTarget isKindOf "CinderWallDoor_DZ_Base" or ((typeOf _cursorTarget) in ["Land_Ind_TankSmall2_EP1","Land_Ind_Garage01","Land_sara_hasic_zbroj","Land_SS_hangar"]);
	_isUnlockedDoor = (_cursorTarget isKindOf "Land_DZE_WoodDoorLocked_Base" or _cursorTarget isKindOf "CinderWallDoorLocked_DZ_Base") && {_cursorTarget animationPhase "Open_hinge" == 1};

	_isModular = (_cursorTarget isKindOf "ModularItems") or ((typeOf _cursorTarget) in ["Land_Ind_TankSmall2_EP1","Land_Ind_Garage01","Land_sara_hasic_zbroj","Land_SS_hangar"]);
	...
	if(_isAlive) then {
		...
		//Allow owners to delete modulars
                if(_isModular && ((dayz_characterID == _ownerID) or _typeOfCursorTarget == "Plastic_Pole_EP1_DZ")) then {
	...
	if(_player_deleteBuild) then {
		if (s_player_deleteBuild < 0) then {
			...
			// ADDED FOR deconstructable
			s_player_deleteBuild = player addAction [format[localize "str_actions_delete",_text], "custom\deconstructible\remove.sqf",_cursorTarget, 1, true, true, "", ""];
		};
	} else {
	...
	*/
	...
	// MF-Tow Script by Matt Fairbrass (matt_d_rat)  // DEPRECATED!
	...
	if (_isMan && !_isPZombie && _traderType in serverTraders) then {
		...
		if (s_player_parts_crtl < 0) then {
			...
			if(_humanity_logic) then {
				...
				_metals_trader = player addAction ["Trade Gems", "custom\currency\trade_metals.sqf",["na"], 0, true, false, "",""];
				s_player_parts set [count s_player_parts,_metals_trader];

				// Add static base kit trader options under sub menu
				//_base_kits = player addAction ["Mobile Base Kits", "custom\currency\buy_basekit.sqf",["na"], 0, true, false, "",""];
				//s_player_parts set [count s_player_parts,_base_kits];
