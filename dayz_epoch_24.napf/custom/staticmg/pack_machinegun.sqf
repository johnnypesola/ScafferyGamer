...
_isRemovable = _objType in ["M2StaticMG", "DSHKM_RU"];
...
if (_proceed) then {
	...
	if(!isNull(_obj)) then {
		...
		if (_objType == "M2StaticMG") then {
			_selectedRemoveOutput = [["100Rnd_127x99_M2",4],["ItemPole",3],["PartGeneric",1]];
		} else {
			_selectedRemoveOutput = [["50Rnd_127x107_DSHKM",4],["ItemPole",3],["PartGeneric",1]];
		};
