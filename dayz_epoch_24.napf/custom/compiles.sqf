...
if (!isDedicated) then {
	...
	fnc_usec_selfActions =			compile preprocessFileLineNumbers "custom\deconstructible\fn_selfActions.sqf";						//Checks which actions for self
	...
	dayz_spaceInterrupt =			compile preprocessFileLineNumbers "custom\gui\dayz_spaceInterrupt.sqf";
	...
	//player_switchModel =		compile preprocessFileLineNumbers "\z\addons\dayz_code\compile\player_switchModel.sqf";	// DZGM
	...
	zombie_generate = 			compile preprocessFileLineNumbers "custom\slow_zombies\zombie_generate.sqf";			//Server compile, used for loiter behaviour
	wild_spawnZombies = 		compile preprocessFileLineNumbers "custom\slow_zombies\wild_spawnZombies.sqf";			//Server compile, used for loiter behaviour
	...
	player_craftItem =			compile preprocessFileLineNumbers "custom\patches\player_craftItem.sqf";
	...
	player_harvestPlant =		compile preprocessFileLineNumbers "custom\plant\player_harvestPlant.sqf";
	...
	//ui
	player_selectSlot =			compile preprocessFileLineNumbers "custom\extra_rc\ui_selectSlot.sqf";	// ADDED: Right click build options // And DZGM
		...
		_total_currency = 0;
		{
			...
			if isNumber (_worth) then {
				...
				switch (_x) do
				{
					case "ItemRuby":
					{
						_total_currency = _total_currency + 100000000;
					};
					case "ItemEmerald":
					{
						_total_currency = _total_currency + 10000000;
					};
					case "ItemSapphire":
					{
						_total_currency = _total_currency + 1000000;
					};
					case "ItemTopaz":
					{
						_total_currency = _total_currency + 100000;
					};
				};
			};
	...
	epoch_itemCost = {
		_trade_total = 0;
		{
			...
			if (isNil "_part_in_configClass") exitWith { 
				diag_log "DEBUG TRADER ITEM WAS null! Exiting...";
			};
			if (isClass (_part_in_configClass)) then {
				...
				if isNumber (_part_inWorth) then {
					...
					switch (_x select 0) do
					{
						case "ItemRuby":
						{
							_trade_total = _trade_total + (100000000 * (_x select 1));
						};
						case "ItemEmerald":
						{
							_trade_total = _trade_total + (10000000 * (_x select 1));
						};
						case "ItemSapphire":
						{
							_trade_total = _trade_total + (1000000 * (_x select 1));
						};
						case "ItemTopaz":
						{
							_trade_total = _trade_total + (100000 * (_x select 1));
						};
					};
				};
			};
	...
	epoch_returnChange =			compile preprocessFileLineNumbers "custom\currency\epoch_returnChange.sqf";
	...
	fnc_usec_damageHandler =		compile preprocessFileLineNumbers "custom\damage\fn_damageHandler.sqf";		//Event handler run on damage
	...
	//local_lockUnlock =			compile preprocessFileLineNumbers "custom\mf-tow\local_lockUnlock.sqf";				//When vehicle is local to unit perform locking vehicle
